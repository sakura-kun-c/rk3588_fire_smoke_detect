...
// MPP 解码器初始化
int MppDecoder::Init(int video_type, int fps, void *userdata, int camera_id)
{
    MPP_RET ret = MPP_OK;
    this->userdata = userdata;
    this->fps = fps;
    this->last_frame_time_ms = 0;
    this->input_id = camera_id;
    ...
    {
        LOGD("unsupport video_type %d \n", video_type);
        return -1;
    }
    // LOGD("mpi_dec_test start ");
    memset(&loop_data, 0, sizeof(loop_data));
    LOGD("mpi_dec_test decoder test start mpp_type %d ", video_type);
    ...
    if (ret)
    {
        LOGD("%p failed to get decoder cfg ret %d \n", mpp_ctx, ret);
        return -1;
    }
    ...
    if (ret)
    {
        LOGD("%p failed to set split_parse ret %d \n", mpp_ctx, ret);
        return -1;
    }
    ...
    if (ret)
    {
        LOGD("%p failed to set cfg %p ret %d \n", mpp_ctx, cfg, ret);
        return -1;
    }
...
int MppDecoder::Decode(uint8_t *pkt_data, int pkt_size, int pkt_eos)
{
    ...
    do
    {
        ...
        try_again:
            ...
            if (MPP_OK != ret)
            {
                LOGD("decode_get_frame failed ret %d \n", ret);
                break;
            }
            ...
            if (frame)
            {
                ...
                if (mpp_frame_get_info_change(frame))
                {
                    ...
                    if (NULL == data->frm_grp)
                    {
                        ...
                        if (ret)
                        {
                            LOGD("%p get mpp buffer group failed ret %d \n", ctx, ret);
                            break;
                        }
                        ...
                        if (ret)
                        {
                            LOGD("%p set buffer group failed ret %d \n", ctx, ret);
                            break;
                        }
                    }
                    else
                    {
                        ...
                        if (ret)
                        {
                            LOGD("%p clear buffer group failed ret %d \n", ctx, ret);
                            break;
                        }
                    }
                    ...
                    if (ret)
                    {
                        LOGD("%p limit buffer group failed ret %d \n", ctx, ret);
                        break;
                    }
                    ...
                    if (ret)
                    {
                        LOGD("%p info change ready failed ret %d \n", ctx, ret);
                        break;
                    }
                ...
                else
                {
                    ...
                    if (callback != nullptr)
                    {
                        ...
                        // LOGD("data_vir=%p fd=%d \n", data_vir, fd);
                        callback(this->userdata, hor_stride, ver_stride, hor_width, ver_height, format, fd, data_vir, this->input_id);
                    }
